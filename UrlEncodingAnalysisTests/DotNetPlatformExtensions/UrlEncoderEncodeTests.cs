...
namespace UrlEncodingAnalysisTests.DotNetPlatformExtensions
{
    /// <summary>
    /// UrlEncoder 類別 Encode 方法的測試類別。
    /// </summary>
    [TestClass]
    public class UrlEncoderEncodeTests
    {
        #region -- RFC 3986 保留字 --

        [TestMethod]
        [TestCategory(nameof(UrlEncoder))]
        [TestProperty(nameof(UrlEncoder), nameof(UrlEncoder.Default.Encode))]
        public void 當輸入為RFC3986保留字的冒號時_必須編碼為百分號3A()
        {
            ...
            // Act
            var actual = UrlEncoder.Default.Encode(input);
        ...
        public void 當輸入為長度65520的字串時_必須無拋出例外()
        {
            ...
            // Act
            Action act = () => UrlEncoder.Default.Encode(input);
        ...
        public void 當輸入為任何的字元字串時_必須無拋出例外()
        {
            ...
            // Act
            Action act = () => allChars.ForEach(c => UrlEncoder.Default.Encode(c.ToString()));
        ...
        public void 當輸入為任何的字元字串時_必須共65536字元皆無拋出例外()
        {
            ...
            allChars.ForEach(c =>
            {
                try
                {
                    UrlEncoder.Default.Encode(c.ToString());
                }
                catch (Exception)
                {
                    return;
