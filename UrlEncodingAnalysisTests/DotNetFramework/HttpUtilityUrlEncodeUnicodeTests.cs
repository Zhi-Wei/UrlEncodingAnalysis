...
{
    /// <summary>
    /// HttpUtility 類別 UrlEncodeUnicode 方法的測試類別。
    /// </summary>
    [TestClass]
    public class HttpUtilityUrlEncodeUnicodeTests
    {
        #region -- RFC 3986 保留字 --

        [TestMethod]
        [TestCategory(nameof(HttpUtility))]
        [TestProperty(nameof(HttpUtility), nameof(HttpUtility.UrlEncodeUnicode))]
        public void 當輸入為RFC3986保留字的冒號時_必須編碼為百分號3A()
        {
            ...
            // Act
            var actual = HttpUtility.UrlEncodeUnicode(input);
        ...
        public void 當輸入為長度65520的字串時_必須無拋出例外()
        {
            ...
            // Act
            Action act = () => HttpUtility.UrlEncodeUnicode(input);
        ...
        public void 當輸入為任何的字元字串時_必須無拋出例外()
        {
            ...
            // Act
            Action act = () => allChars.ForEach(c => HttpUtility.UrlEncodeUnicode(c.ToString()));
        ...
        public void 當輸入為任何的字元字串時_必須共65536字元皆無拋出例外()
        {
            ...
            allChars.ForEach(c =>
            {
                try
                {
                    HttpUtility.UrlEncodeUnicode(c.ToString());
                }
                catch (Exception)
                {
                    return;
