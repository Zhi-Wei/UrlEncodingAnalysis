...
namespace UrlEncodingAnalysisTests.FlurlLibrary
{
    /// <summary>
    /// Url 類別 EncodeIllegalCharacters 方法的測試類別。
    /// </summary>
    [TestClass]
    public class UrlEncodeIllegalCharactersTests
    {
        #region -- RFC 3986 保留字 --

        [TestMethod]
        [TestCategory(nameof(Url))]
        [TestProperty(nameof(Url), nameof(Url.EncodeIllegalCharacters))]
        public void 當輸入為RFC3986保留字的冒號時_必須編碼為百分號3A()
        {
            ...
            // Act
            var actual = Url.EncodeIllegalCharacters(input);
        ...
        public void 當輸入為長度65520的字串時_必須無拋出例外()
        {
            ...
            // Act
            Action act = () => Url.EncodeIllegalCharacters(input);
        ...
        public void 當輸入為任何的字元字串時_必須無拋出例外()
        {
            ...
            // Act
            Action act = () => allChars.ForEach(c => Url.EncodeIllegalCharacters(c.ToString()));
        ...
        public void 當輸入為任何的字元字串時_必須共65536字元皆無拋出例外()
        {
            ...
            allChars.ForEach(c =>
            {
                try
                {
                    Url.EncodeIllegalCharacters(c.ToString());
                }
                catch (Exception)
                {
                    return;
